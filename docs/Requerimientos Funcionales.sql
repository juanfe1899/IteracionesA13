...
--RFC4:
 ...
 -- RFC 10
SELECT CLIENTES.*, N_CLIENTE.CANTIDAD_COMPRADA
FROM(
        select  FACTURAS.ID_CLIENTE, PRODUCTOS_SUCURSAL.ID, SUM(VENTAS_PRODUCTO.CANTIDAD) CANTIDAD_COMPRADA
        from    FACTURAS, VENTAS_PRODUCTO, PRODUCTOS_SUCURSAL
        WHERE FACTURAS.ID = VENTAS_PRODUCTO.ID_FACTURA
              AND VENTAS_PRODUCTO.ID_PRODUCTO_SUC = PRODUCTOS_SUCURSAL.ID
              AND PRODUCTOS_SUCURSAL.ID_PRODUCTO = 1
              AND FACTURAS.FECHA BETWEEN TO_DATE('10-JUN-2018','DD-MON-YYYY') AND TO_DATE('10-OCT-2018','DD-MON-YYYY')
        GROUP BY (PRODUCTOS_SUCURSAL.ID, FACTURAS.ID_CLIENTE)
    ) N_CLIENTE,( CLIENTES)
    WHERE CLIENTES.ID = N_CLIENTE.ID_CLIENTE 
;
...
FROM (CLIENTES) LEFT JOIN(
    ...
    ON CLIENTES.ID = N_CLIENTE.ID_CLIENTE
    WHERE n_cliente.ID_CLIENTE is null
          and CLIENTES.ID = N_CLIENTE.ID_CLIENTE 
;
select CLIENTES.* from clientes;
select * from facturas;

-- RFC12
    -- PRODUCTOS MAS (M) Y MENOS (N) VENDIDOS POR SEMANA EN EL ANIO 2018
SELECT SEMANA, PRODUCTO, UNI_VENDIDAS
FROM(
    SELECT SEMANA, 
           PRODUCTO, 
           UNI_VENDIDAS, 
           MAX(UNI_VENDIDAS) OVER (PARTITION BY SEMANA) AS M, 
           MIN(UNI_VENDIDAS) OVER (PARTITION BY SEMANA) AS N
    FROM(
            SELECT FLOOR(TO_CHAR(FECHA, 'DDD')/7) SEMANA, 
                   PRODUCTOS_SUCURSAL.ID_PRODUCTO PRODUCTO, 
                   SUM(VENTAS_PRODUCTO.CANTIDAD) UNI_VENDIDAS--, MAX(SUM(VENTAS_PRODUCTO.CANTIDAD)) MA, MIN(SUM(VENTAS_PRODUCTO.CANTIDAD)) ME
            FROM FACTURAS, VENTAS_PRODUCTO, PRODUCTOS_SUCURSAL
            WHERE FACTURAS.ID = VENTAS_PRODUCTO.ID_FACTURA
                  AND VENTAS_PRODUCTO.ID_PRODUCTO_SUC = PRODUCTOS_SUCURSAL.ID,
                  AND TO_CHAR(FECHA, 'YYYY') = '2018'
            GROUP BY FLOOR(TO_CHAR(FECHA, 'DDD')/7), PRODUCTOS_SUCURSAL.ID_PRODUCTO
                )
    ORDER BY SEMANA, UNI_VENDIDAS
    )
WHERE UNI_VENDIDAS = M
        OR UNI_VENDIDAS = N;
   
    --PROVEEDORES MAS Y MENOS SOLICITADOS POR SEMANA EN EL ANIO 2018
SELECT 
    SEMANA,
    ID_PROVEEDOR,
    N_PEDIDOS
FROM(
    SELECT 
        A.*, 
        MAX(A.N_PEDIDOS) over (partition by A.SEMANA) MENOR, 
        MIN(A.N_PEDIDOS) over (partition by A.SEMANA) MAYOR
    FROM (
        SELECT FLOOR(TO_CHAR(FECHA_CREACION, 'DDD')/7) SEMANA,
               ID_PROVEEDOR, 
               COUNT(ID) N_PEDIDOS
        FROM PEDIDOS_SUCURSAL
        WHERE TO_CHAR(FECHA_CREACION, 'YYYY') = '2018'
        GROUP BY FLOOR(TO_CHAR(FECHA_CREACION, 'DDD')/7), ID_PROVEEDOR
        ) A
    )  DEMANDAS
WHERE DEMANDAS.N_PEDIDOS = DEMANDAS.MENOR
    OR DEMANDAS.N_PEDIDOS = DEMANDAS.MAYOR
ORDER BY SEMANA, N_PEDIDOS;


-- RFC13
    -- CLIENTES QUE COMPRAN AL MENOS UNA VEZ AL MESEN EL ANIO <2018>
SELECT ID_CLIENTE, N_FACTURAS_ANIO
FROM(
    SELECT 
        ID_CLIENTE, 
        SUM(N_FACTURAS) N_FACTURAS_ANIO, 
        COUNT(FECHA) N_MESES
    FROM(
        SELECT TO_CHAR(FECHA, 'MM-YYYY') FECHA, ID_CLIENTE, COUNT(ID) N_FACTURAS
        FROM FACTURAS
        WHERE TO_CHAR(FECHA, 'YYYY') = '2018'
        GROUP BY TO_CHAR(FECHA, 'MM-YYYY'), ID_CLIENTE
        ) A
    GROUP BY ID_CLIENTE
    )
WHERE N_MESES = '12'
...
-- ID DEL CLIENTE QUE SIEMPRE COMPRA HERRAMIENTAS. JUNTO CON EL TOTAL DE FACTURAS DE SUS COMPRAS.
SELECT TOT.ID_CLIENTE, TOTAL_FACTURAS
FROM(
SELECT ID_CLIENTE, COUNT(ID) TOTAL_FACTURAS
FROM FACTURAS
GROUP BY ID_CLIENTE
    ) TOT ,(
SELECT FACTURAS.ID_CLIENTE, COUNT(FACTURAS.ID) NUM_FACTURAS_X --id_cliente con el NUMERO de facturas que incluyen alguna herramienta
FROM FACTURAS, (
    SELECT DISTINCT ID_FACTURA
    FROM VENTAS_PRODUCTO, (
        SELECT
            PRODUCTOS_SUCURSAL.ID ID_PRODUCTO
        FROM PRODUCTOS, PRODUCTOS_SUCURSAL, CATEGORIAS
        WHERE PRODUCTOS.CODIGO = PRODUCTOS_SUCURSAL.ID_PRODUCTO
              AND PRODUCTOS.CATEGORIA = CATEGORIAS.ID
              AND CATEGORIAS.NOMBRE = 'HERRAMIENTA'
        ) HERRAMIENTAS
    WHERE VENTAS_PRODUCTO.ID_PRODUCTO_SUC = HERRAMIENTAS.ID_PRODUCTO
    ) FACTURA_HERRAMIENTA
WHERE  FACTURAS.ID = FACTURA_HERRAMIENTA.ID_FACTURA
GROUP BY FACTURAS.ID_CLIENTE
    ) BUSCADAS
WHERE BUSCADAS.NUM_FACTURAS_X = TOT.TOTAL_FACTURAS;


-- ID DEL CLIENTE que siempre compran al menos un producto costoso. JUNTO CON EL TOTAL DE FACTURAS DE SUS COMPRAS.
SELECT TOT.ID_CLIENTE, TOTAL_FACTURAS
FROM(
    SELECT ID_CLIENTE, COUNT(ID) TOTAL_FACTURAS
    FROM FACTURAS
    ...
    SELECT FACTURAS.ID_CLIENTE, COUNT(FACTURAS.ID) NUM_FACTURAS_X --id_cliente con el NUMERO de facturas que incluyen producto costoso
    FROM FACTURAS, (
        SELECT DISTINCT ID_FACTURA
        FROM VENTAS_PRODUCTO, (
            SELECT ID
            FROM PRODUCTOS_SUCURSAL
            WHERE PRECIO_CU >= '100000'
            ) PRODUCT_COSTOSO
        WHERE VENTAS_PRODUCTO.ID_PRODUCTO_SUC = PRODUCT_COSTOSO.ID
        ) FACTURA_HERRAMIENTA
    WHERE  FACTURAS.ID = FACTURA_HERRAMIENTA.ID_FACTURA
    GROUP BY FACTURAS.ID_CLIENTE
    ) BUSCADAS
